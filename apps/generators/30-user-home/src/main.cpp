...
int main() // NOLINT
{
    ...
    auto mountDir = [&mounts](const std::string &hostDir, const std::string &containerDir) {
        std::error_code ec;
        // if directory already exist, it will do nothing
    ...
    std::error_code ec;
    auto oldAppDir = hostHomeDir / ".linglong" / appID;

    bool hasOldAppDir = std::filesystem::exists(oldAppDir, ec);
    if (ec) {
        std::cerr << "failed to get state of " << oldAppDir << ": " << ec.message() << std::endl;
        return -1;
    }
    ...
    if (hasOldAppDir) {
        XDGConfigHome = oldAppDir / "config";
    }
    ...
    auto oldStateDir = oldAppDir / "state";
    if (std::filesystem::exists(oldStateDir, ec)) {
        XDGStateHome = oldStateDir;
    }
    ec.clear();
    ...
    // only hide some directory for newly installed application
    if (hasOldAppDir) {
        std::cout << content.dump() << std::endl;
        return 0;
    }

    auto appHiddenDir = XDGDataHome / "linglong" / appID / ".hidden";
    if (!std::filesystem::exists(appHiddenDir, ec)) {
        if (ec) {
            std::cerr << "failed to get state of " << appHiddenDir.c_str() << ": " << ec.message()
                      << std::endl;
            return -1;
        }

        if (!std::filesystem::create_directories(appHiddenDir, ec)) {
            std::cerr << "failed to create " << appHiddenDir.c_str() << ": " << ec.message()
                      << std::endl;
            return -1;
        }
    }

    // hide self configurations
    auto linglongConfigDir = XDGConfigHome / "linglong";
    auto linglongDataDir = XDGDataHome / "linglong";
    auto linglongHiddenDir = linglongDataDir / ".hidden";
    if (!mountDir(linglongHiddenDir / "config", cognitiveConfigHome / "linglong")) {
        return -1;
    }

    if (!mountDir(linglongHiddenDir / "data", cognitiveDataHome / "linglong")) {
        return -1;
    }

    auto privileges = linglong::api::types::v1::ApplicationAccessPrivileges{};
    auto config = linglongConfigDir / "apps" / (appID + ".json");
    ...
    for (const std::filesystem::path relative : disallowedDirs) {
        ...
        if (!mountDir(appHiddenDir / relative, cognitiveHomeDir / relative)) {
            return -1;
        }
    }
