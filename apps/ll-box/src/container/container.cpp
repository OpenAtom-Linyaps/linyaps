                          ...
                          int initPid)
{
    ...
    auto create_directories =
      [](const std::filesystem::path &path, std::filesystem::perms perm, std::error_code &ec) {
          ...
          std::filesystem::permissions(path, perm, std::filesystem::perm_options::replace, ec);
          if (ec) {
              logErr() << "set " << path.string() << "permission error:" << ec.message();
              return;
          }
      };

    const auto dirPerm = std::filesystem::perms::owner_all | std::filesystem::perms::group_read
    ...
    if (auto ret = ::mount("cgroup2", cgroupsRoot.string().c_str(), "cgroup2", 0U, nullptr);
        ret == -1) {
...
int Container::NonePrivilegeProc(void *self)
{
    ...
    if (container->runtime.hooks.has_value()) {
        ...
        for (auto const &startContainer :
             container->runtime.hooks->startContainer.value_or(std::vector<Hook>{})) {
...
int Container::EntryProc(void *self)
{
    ...
    if (container->useNewCgroupNs) {
        auto ret = ConfigCgroupV2(container->runtime.linux.cgroupsPath,
                                  container->runtime.linux.resources,
                                  getpid());
        if (ret != 0) {
    ...
    int noPrivilegePid =
      util::PlatformClone(&Container::NonePrivilegeProc, nonePrivilegeProcFlag, self);
    if (noPrivilegePid < 0) {
...
Container::Container(std::filesystem::path bundle, std::string id, Runtime runtime)
...
int Container::PrepareDefaultDevices()
{
    ...
    if (ec) {
        logErr() << "create symlink from" << hostPtmx.string() << "to" << containerPtmx.string()
                 << "error:" << ec.message();
        return -1;
    }
...
int Container::MountContainerPath()
{
    if (runtime.mounts.has_value()) {
        for (auto &mount : runtime.mounts.value()) {
            ...
            if (!containerMounter.MountNode(mount)) {
                logWan() << "failed to Mount:" << mount.source << "to" << mount.destination;
            }
        }
    };
