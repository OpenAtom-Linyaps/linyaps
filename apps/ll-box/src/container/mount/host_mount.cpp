...
bool do_mount_with_fd(const std::string &root,
                      ...
                      const void *data) noexcept
{
    ...
    if (fd < 0) {
        logErr() << "fail to open destination " << destination << linglong::util::errnoString();
        return false;
    }

    linglong::util::defer closeFd{ [fd] {
        ::close(fd);
    } };
    ...
    if (ec) {
        logErr() << "failed to read symlink " << target.string() << " error:" << ec.message();
        return false;
    }
    ...
    int ret{ -1 };
    if (ret = ::mount(source.c_str(), target.c_str(), filesystemType.data(), mountFlags, data);
        ret == 0) {
        return true;
    }

    logErr() << "mount" << source << "to" << realPathStr
             << "failed:" << linglong::util::RetErrString(ret) << "\nmount args: filesystemType ["
             << filesystemType << "], mountFlag [" << mountFlags << "], data:" << data;
    return false;
}
...
std::filesystem::path
HostMount::toHostDestination(const std::filesystem::path &containerDestination) noexcept
{
...
bool HostMount::ensureDirectoryExist(const std::filesystem::path &destination) noexcept
{
    std::error_code ec;
    ...
    if (ec) {
        logErr() << "couldn't check existence of directory" << destination << ":" << ec.message();
        return false;
    }

    if (!std::filesystem::create_directories(destination, ec) && ec) {
        logErr() << "couldn't create directories" << destination.string() << ":" << ec.message();
        return false;
    }

    using perms = std::filesystem::perms;
    auto perm = perms::owner_all | perms::group_read | perms::group_exec | perms::others_read
      | perms::others_exec; // 0755
    std::filesystem::permissions(destination, perm, std::filesystem::perm_options::replace, ec);
    if (ec) {
        logErr() << "set " << destination.string() << "permission error:" << ec.message();
        return false;
    }
...
bool HostMount::ensureFileExist(const std::filesystem::path &destination) noexcept
{
    std::error_code ec;
    if (std::filesystem::exists(destination, ec)) {
        return true;
    ...
    if (ec) {
        logErr() << "check" << destination.string() << "exist error:" << ec.message();
        return false;
    }
...
void HostMount::finalizeMounts() const
{
    for (const auto &node : remountList) {
        auto targetPath = std::filesystem::path("/proc/self/fd") / std::to_string(node.targetFd);
        if (!remount(targetPath, node.flags, node.data)) {
            logWan() << "failed to remount:" << targetPath.string() << util::errnoString();
        }

        if (::close(node.targetFd) == -1) {
            logWan() << "failed to close fd:" << node.targetFd << " " << util::errnoString();
        }
    }
}

std::optional<bool> HostMount::isDummy(const std::string &filesystemType) noexcept
{
...
bool HostMount::remount(const std::filesystem::path &target,
                        uint32_t flags,
                        const std::string &data)
{
    ...
    auto remount_flags = static_cast<uint32_t>(sfs.f_flags) & (MS_NOSUID | MS_NODEV | MS_NOEXEC);
    if ((flags | remount_flags) != flags) {
        if (::mount(nullptr, target.c_str(), nullptr, flags | remount_flags, data_ptr) == 0) {
            return true;
        }
        ...
        if ((sfs.f_flags & MS_RDONLY) != 0) {
            ...
            if (::mount(nullptr, target.c_str(), nullptr, flags | remount_flags, data_ptr) == -1) {
...
bool HostMount::MountNode(const struct Mount &m)
{
    ...
    auto sourceStatus = std::filesystem::symlink_status(source, ec); // same as lstat
    if (ec) {
    ...
    case std::filesystem::file_type::symlink: {
        if (!ensureDirectoryExist(destination.parent_path())) {
            logErr() << "failed to ensure the parent directory of host destination exist.";
            return false;
        }
        ...
        if (nosymfollow) {
            ...
            source = std::filesystem::path{ "/proc/self/fd" } / std::to_string(sourceFd);
            break;
        }
    ...
    case std::filesystem::file_type::not_found: {
        ...
        if (dummy && !dummy.value()) {
            logErr() << "try to mount a not existing source" << source.string() << "with filesystem"
                     << m.type;
            return false;
        }
    ...
    const uint32_t all_propagations = (MS_SHARED | MS_PRIVATE | MS_SLAVE | MS_UNBINDABLE);
    ...
    case Mount::Bind: {
        ...
        // remounting.
        real_data.clear();
        if (!do_mount_with_fd(containerRoot, source, destination, "", real_flags, nullptr)) {
            break;
        }
        ...
        if ((m.propagationFlags & all_propagations) != 0U) {
            ...
            if (propagation != 0
                && !do_mount_with_fd(containerRoot,
                                     "",
                                     destination,
                                     "",
                                     rec | propagation,
        ...
        if (newFd == -1) {
            logErr() << "failed to open " << destination.string() << ", abort remount.";
            break;
        }
    ...
    case Mount::Devpts:
        [[fallthrough]];
    case Mount::Tmpfs: {
        if (do_mount_with_fd(containerRoot,
                             source,
                             destination,
                             m.type,
                             real_flags,
    ...
    case Mount::Mqueue: {
        ...
        if (do_mount_with_fd(containerRoot, "/dev/mqueue", destination, "", real_flags, nullptr)) {
            return true;
        }
    } break;
    case Mount::Sysfs: {
        ...
        // refers: Mqueue
        real_flags = MS_BIND | MS_REC;
        if (do_mount_with_fd(containerRoot, "/sys", destination, "", real_flags, nullptr)) {
            sysfs_is_binded = true;
            return true;
        }
    } break;
    ...
    if (auto str = source.string(); !str.empty() && str.at(0) == '/') {
        logErr() << "source file type is: " << to_string(sourceStatus.type())
                 << ", permission:" << std::oct << static_cast<int>(sourceStatus.permissions());
