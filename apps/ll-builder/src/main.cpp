...
public:
     ...
     */
    EnvironmentVariableGuard(const std::string &variableName, const std::string &newValue)
...
int handle_create(const CreateCommandOptions &options)
{
...
int handle_build(linglong::builder::Builder &builder, const BuildCommandOptions &options)
{
...
int handle_run(linglong::builder::Builder &builder, const RunCommandOptions &options)
{
...
int handle_export(linglong::builder::Builder &builder, const ExportCommandOptions &options)
{
...
int handle_push(linglong::builder::Builder &builder, const PushCommandOptions &options)
{
...
int handle_list(linglong::repo::OSTreeRepo &repo,
                [[maybe_unused]] const ListCommandOptions &options)
{
...
int handle_remove(linglong::repo::OSTreeRepo &repo, const RemoveCommandOptions &options)
{
...
int handle_import(linglong::repo::OSTreeRepo &repo, const ImportCommandOptions &options)
{
...
int handle_import_dir(linglong::repo::OSTreeRepo &repo, const ImportDirCommandOptions &options)
{
...
int handle_extract(const ExtractCommandOptions &options)
{
    QString layerFile = QString::fromStdString(options.layerFile);
...
int handle_repo_show(linglong::repo::OSTreeRepo &repo)
{
...
int handle_repo_add(linglong::repo::OSTreeRepo &repo, linglong::cli::RepoOptions &options)
{
...
int handle_repo_remove(linglong::repo::OSTreeRepo &repo, linglong::cli::RepoOptions &options)
{
    auto newCfg = repo.getConfig();
...
int handle_repo_update(linglong::repo::OSTreeRepo &repo, linglong::cli::RepoOptions &options)
{
    auto newCfg = repo.getConfig();
    const std::string &alias = options.repoAlias.value();
...
int handle_repo_set_default(linglong::repo::OSTreeRepo &repo, linglong::cli::RepoOptions &options)
{
    auto newCfg = repo.getConfig();
    const std::string &alias = options.repoAlias.value();
...
int handle_repo(linglong::repo::OSTreeRepo &repo,
                ...
                CLI::App *buildRepoSetDefault)
{
    ...
    if (buildRepoAdd->parsed()) {
        return handle_repo_add(repo, repoOptions);
    }

    if (buildRepoRemove->parsed()) {
        return handle_repo_remove(repo, repoOptions);
    }

    if (buildRepoUpdate->parsed()) {
        return handle_repo_update(repo, repoOptions);
    }

    if (buildRepoSetDefault->parsed()) {
        return handle_repo_set_default(repo, repoOptions);
    }
...
You can report bugs to the linyaps team under this project: https://github.com/OpenAtom-Linyaps/linyaps/issues)");
    ...
    if (buildCreate->parsed()) {
        return handle_create(createOpts);
    }

    if (buildExtract->parsed()) {
        return handle_extract(extractOpts);
    }
    ...
    auto gioGuard = std::make_unique<EnvironmentVariableGuard>("GIO_USE_VFS", "local");
    auto gvfsGuard =
      std::make_unique<EnvironmentVariableGuard>("GVFS_REMOTE_VOLUME_MONITOR_IGNORE", "1");
    ...
    if (buildImport->parsed()) {
        return handle_import(repo, importOpts);
    }

    if (buildImportDir->parsed()) {
        return handle_import_dir(repo, importDirOpts);
    }

    if (buildList->parsed()) {
        return handle_list(repo, listOpts);
    }

    if (buildRemove->parsed()) {
        return handle_remove(repo, removeOpts);
    }
    ...
    if (buildBuilder->parsed()) {
        return handle_build(builder, buildOpts);
    }

    if (buildRun->parsed()) {
        return handle_run(builder, runOpts);
    }

    if (buildExport->parsed()) {
        return handle_export(builder, exportOpts);
    }
    ...
    if (buildPush->parsed()) {
        ...
        return handle_push(builder, pushOpts);
    }
