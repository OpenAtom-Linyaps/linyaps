...
ll-cli list --upgradable
    ...
    // add sub command dir
    auto cliLayerDir =
      commandParser.add_subcommand("dir", "Get the layer directory based on the application ID")
        ->group(CliHiddenGroup);
    cliLayerDir->add_option("APP", options.appid, _("Specify the installed application ID"))
      ->required()
      ->check(validatorString);
