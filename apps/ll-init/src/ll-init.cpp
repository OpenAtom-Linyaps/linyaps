...
int delegate_run(int argc, char **argv) noexcept
{
    ...
    if (child == 0) {
        auto ret = ::signal(SIGHUP, SIG_IGN);
        if (ret == SIG_ERR) {
            print_sys_error("failed to ignore sighup");
            _exit(EXIT_FAILURE);
        }
        ...
        if (ret == SIG_ERR) {
            print_sys_error("failed to reset SIGHUP handler");
            _exit(EXIT_FAILURE);
        }
    ...
    if (::signal(SIGHUP, SIG_IGN) == SIG_ERR) {
        print_sys_error("failed to ignore SIGHUP in parent process");
        ::kill(child, SIGKILL);
    }

    if (stdin_is_tty) {
        if (::ioctl(STDIN_FILENO, TIOCNOTTY, 0) == -1) {
            print_sys_error("TIOCNOTTY failed");
        }
    }
