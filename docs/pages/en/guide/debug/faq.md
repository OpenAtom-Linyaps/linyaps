...
7. Why does the desktop shortcut icon appear as a gear or blank? How are the icon files placed?
...
8. Why do `xdg-open` and `xdg-email` that come with the application fail?
...
9. Why doesn't the system environment variable used by the application take effect?
...
10. The library files required for the application to run were not found. How can I provide them?
...
11. Why has the `Qt WebEngine` rendering process crashed when the application is running?
...
12. When the application is running, the `libqxcb.so` library cannot be found or the `qtwebengine` error is reported. What can I do?
...
13. Can the application carry the database file by itself and write data to the database during operation?
...
14. Why does the execution of binary with `suid` and `guid` permissions fail?
...
15. Can the input method of uab offline package format not be used under Debian and Ubuntu?

    It is recommended to install the `fictx` input method to experience it.
16. How can I know which packages are installed in a container environment?
