...
**Tips: 此文档适用于1.6.3以下的如意玲珑版本，1.6.3以上的版本可参考文档：[转换appimage应用包格式](../ll-appimage-convert/convert-appimage.md)**

# 转换 `appimage` 包格式( `.appimage` 或 `.AppImage` ) 到如意玲珑包格式( `.layer` 或 `.uab` )

使用 `ll-builder convert` 转换 appimage 包格式( `.appimage` 或 `.AppImage` )为如意玲珑包格式( `.layer` 或 `.uab` )
...
`ll-builder convert` 命令会根据指定的应用名称（ `--name` 选项）生成一个目录，该目录会作为如意玲珑项目的根目录，即 `linglong.yaml` 文件所在的位置。它支持两种转换方法：
...
指定要转换的如意玲珑包的相关参数，稍等片刻后你就可以得到 `io.github.brainwaves_0.15.1_x86_64_runtime.layer` 或者 `io.github.brainwaves_0.15.1_x86_64_runtime.uab` 包文件。
