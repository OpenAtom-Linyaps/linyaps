...
Ubuntu 24.04 也可以使用 DEB822 源格式。你可以创建一个 `.sources` 文件来写入同样的软件源信息：

```sh
sudo tee /etc/apt/sources.list.d/linglong.sources > /dev/null <<'EOF'
Types: deb
URIs: https://ci.deepin.com/repo/obs/linglong:/CI:/release/Ubuntu_24.04/
Suites: ./
Trusted: yes
EOF
sudo apt update
