...
* **调试模式：** `ll-cli run` 新增 `--debug` 模式，通过 gdbserver 启动应用，并支持 `--debug-listen`、`--debug-debuginfod` 和 `--debug-symbol-dir` 选项，便于直接连接调试器。
* **依赖与体积分析：** 新增 `ll-cli analyze depends` 和 `ll-cli analyze size` 子命令，可查看已安装应用的依赖树，并统计模块的独占、共享、逻辑和实际磁盘占用。
* **PipeWire 支持：** 新增 `--enable-pipewire` 选项，可在沙箱中挂载当前用户的 PipeWire socket，为应用提供音频和视频服务访问。
* **AT SPI 支持：** 新增 `--enable-atspi` 选项，可在沙箱中挂载 AT SPI socket，改善辅助功能服务访问。
* **运行时配置增强：** 支持 `config.d/*.json` 增量配置和 CDI 设备配置，可在运行时配置中声明需要挂载的设备。
* **进程列表增强：** `ll-cli ps` 新增 `--no-truncated` 选项，可显示完整容器 ID。
* **版本信息增强：** 非发布构建的版本字符串现在包含 git 哈希和构建通道等元数据，便于定位构建来源。

### 🐞 **问题修复**

* **模块保留：** 修复应用升级或降级时已安装模块丢失的问题。
* **UAB 导出：** 修复 UAB 导出时跨设备硬链接失败导致导出失败的问题，新增复制回退逻辑。
* **调试安全：** 默认将 gdbserver 监听地址限制为回环地址，避免调试服务暴露到网络。
* **导出路径：** 修复相对 `--output` 路径无法正确解析的问题。
* **用户目录：** 使用 `HOME` 环境变量替代硬编码的 `/home/` 路径前缀，修复非默认用户目录下的运行问题。
* **D-Bus 修复：** 修复 QtDBus 文件描述符传递和 `ll-package-manager` 异常启动问题。
* **初始化：** 修复 `ll-init` 处理空命令参数时异常的问题。
* **overlayfs：** 改进内核 overlayfs 支持检测，并修复相关运行模式解析问题。
* **缓存清理：** 修复修改目录权限后清理缓存可能失败的问题。
* **任务状态：** 修复任务快速结束导致 CLI 无法获得最终状态的问题。
* **网络配置：** 修复 `/etc/resolv.conf` 符号链接解析，确保沙箱内使用正确的 DNS 配置。
...
* **实例配置：** `ll-cli run` 新增 `--instance` 选项，可加载应用指定的实例运行配置，并支持自定义挂载。
* **系统级运行时配置：** 新增 `/etc/linglong` 系统级运行时配置，支持系统默认配置、应用配置和用户配置按优先级合并。
* **运行 Runtime/Base 环境：** `ll-cli run` 支持直接在 runtime 或 base 环境中运行命令。
* **NVIDIA GPU 自动检测：** 未显式指定 CDI 设备时自动检测 NVIDIA GPU 并启用 CDI 支持。
* **包管理器授权：** 为 `ll-package-manager` 增加 polkit 授权处理，提升安装、卸载等操作的权限管理能力。

### 🐞 **问题修复**

* **构建运行身份：** 修复 `ll-builder` 运行环境未使用当前用户 UID/GID 的问题。
* **扩展解析：** 修复扩展定义无法从运行配置正确解析的问题。
* **XDP 兼容性：** 对不符合 XDG Desktop Portal ID 规范的应用自动禁用 XDP 集成，并修复文档挂载选项。
* **构建依赖：** 改进构建时依赖拉取逻辑，降低构建失败概率。
