...
include /opt/apps/@id@/files/etc/ld.so.conf)";
    ...
    if (this->project.package.kind != "runtime") {
        ...
        const std::filesystem::path exportDirConfigPath = LINGLONG_DATA_DIR "/builder/export/export-dirs.json";
        if (!std::filesystem::exists(exportDirConfigPath)) {
            return LINGLONG_ERR(QString{"this export config file doesn't exist: %1"}.arg(exportDirConfigPath.c_str()));
        }
        auto exportDirConfig = linglong::utils::serialize::LoadJSONFile<linglong::api::types::v1::BuilderExportDirs>(exportDirConfigPath);
        if (!exportDirConfig) {
            return LINGLONG_ERR(QString{"failed to load export config file: %1"}.arg(exportDirConfigPath.c_str()));
        }
        ...
        for (const auto &item : exportDirConfig->exportPaths) {
            const auto &path = QString::fromStdString(item.path);
