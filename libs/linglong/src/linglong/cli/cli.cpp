...
int Cli::exportUab([[maybe_unused]] CLI::App *subcommand)
{
    ...
    QFileInfo uabFileInfo(uabFileName);
    if (!options.filePath.empty()) {
        uabFileInfo.setFile(QDir::currentPath() + "/" + options.filePath.c_str());
    }

    auto params = api::types::v1::PackageManager1ExportParameters{ .appID = options.appid,
                                                                   .full = options.full,
    ...
    QDBusReply<QString> authReply = this->authorization();
    if (!authReply.isValid() && authReply.error().type() == QDBusError::AccessDenied) {
        auto path = uabFileInfo.absoluteFilePath();
        std::cout << "Access denied. Attempting to write to " << path.toStdString() << std::endl;
    ...
    auto pendingReply =
      this->pkgMan.Export(utils::serialize::toQVariantMap(params));
    pendingReply.waitForFinished();
    ...
    std::filesystem::path destPath = uabFileInfo.absoluteFilePath().toStdString().c_str();
    try {
        std::filesystem::rename(exportPath, destPath);
    ...
    // 移除export 目录
    try {
        std::filesystem::remove_all(exportPath.parent_path());
    } catch (const std::filesystem::filesystem_error &e) {
        std::cerr << "Failed to remove export directory: " << e.what() << std::endl;
    }
