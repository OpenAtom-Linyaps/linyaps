...
void Cli::printProgress() noexcept
{
    ...
    if (this->lastState == api::types::v1::State::Failed) {
        ...
        case utils::error::ErrorCode::AppUpgradeNotFound:
            this->printer.printMessage(_(
              "Upgrade failed, app not installed"));
            break;
        case utils::error::ErrorCode::AppUpgradeLatestInstalled:
            this->printer.printMessage(_(
              "Upgrade failed, latest version already installed"));
            break;
        default:
...
int Cli::install([[maybe_unused]] CLI::App *subcommand)
{
    ...
    if (resultCode != utils::error::ErrorCode::Success) {
        ...
        if (resultCode == utils::error::ErrorCode::AppInstallAlreadyInstalled) {
            this->printer.printMessage(_("Application already installed"));
        }

        if (resultCode == utils::error::ErrorCode::AppInstallNotFoundFromRemote) {
            this->printer.printMessage(QString{ _("Application %1 not found in remote repo.") }.arg(
              params.package.id.c_str()));
        }

        if (resultCode == utils::error::ErrorCode::AppInstallModuleNoVersion) {
            this->printer.printMessage(_("Cannot specify a version when installing a module"));
        }

        if (resultCode == utils::error::ErrorCode::AppInstallNeedDowngrade) {
        ...
        if (resultCode == utils::error::ErrorCode::Unknown
            || resultCode == utils::error::ErrorCode::AppInstallFailed) {
            this->printer.printMessage(
              _("Install failed, internal error.\n Please add --verbose option, and feedback error "
                "information to the developer"));
        }
...
int Cli::uninstall([[maybe_unused]] CLI::App *subcommand)
{
    ...
    if (resultCode != utils::error::ErrorCode::Success) {
        ...
        if (resultCode == utils::error::ErrorCode::AppUninstallNotFoundFromLocal) {
            this->printer.printMessage(QString{ _("Application %1 not found.") }.arg(fuzzyRef->id));
        }

        if (resultCode == utils::error::ErrorCode::AppUninstallFailed
            || resultCode == utils::error::ErrorCode::Unknown) {
            this->printer.printMessage(_(
              "Uninstall failed, internal error.\n Please add --verbose option, and feedback error "
              "information to the developer"));
        }
