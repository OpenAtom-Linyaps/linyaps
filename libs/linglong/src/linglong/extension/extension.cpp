                           ...
                           const std::string &extensionName)
{
    ...
    std::unordered_map<std::string, std::filesystem::path> hostDirMap;
    std::unordered_set<std::string> hostDirBindTargets;
    int hostDirIndex = 0;
    auto ensureHostDir = [&](const std::filesystem::path &sourceDir)
      -> std::optional<std::filesystem::path> {
        if (sourceDir.empty() || !sourceDir.is_absolute()) {
            return std::nullopt;
        }
        auto key = sourceDir.lexically_normal().string();
        auto it = hostDirMap.find(key);
        if (it != hostDirMap.end()) {
            return it->second;
        }
        auto rel = std::filesystem::path("host") / std::to_string(hostDirIndex++);
        hostDirMap.emplace(key, rel);
        std::error_code ec;
        std::filesystem::create_directories(root / rel, ec);
        ec.clear();
        auto dest = std::filesystem::path("/opt/extensions") / extensionName / rel;
        if (hostDirBindTargets.insert(dest.string()).second) {
    ...
    auto linkHostFile = [&](const std::filesystem::path &dest,
                            const std::filesystem::path &source) {
        if (dest.empty() || source.empty()) {
            return;
        }
        auto relDir = ensureHostDir(source.parent_path());
        if (!relDir) {
            return;
        }
        std::filesystem::path target =
          std::filesystem::path("/opt/extensions") / extensionName / *relDir / source.filename();
        std::error_code ec;
        ...
        if (ec) {
            LogW("failed to create host link {} -> {}: {}",
                 dest.string(),
                 target.string(),
                 ec.message());
            ec.clear();
    ...
    extraBindTargets.reserve(libs.size() + 8);
    for (const auto &lib : libs) {
        if (lib.name.empty()) {
            continue;
        }
        ...
        linkHostFile(destPath, lib.source);
    ...
    if (auto nvidiaSmi = findNvidiaSmiPath(libs)) {
        ...
        std::filesystem::path destInRoot = root / rel;
        std::filesystem::create_directories(destInRoot.parent_path(), ec);
        linkHostFile(destInRoot, *nvidiaSmi);
    ...
    auto addMissingLibBinds =
      [&linkHostFile, &root](const std::vector<std::filesystem::path> &missingLibs) {
          for (const auto &missingLib : missingLibs) {
              ...
              std::filesystem::create_directories(destInRoot.parent_path(), ec);
              ec.clear();
              linkHostFile(destInRoot, resolved);
          }
      };
