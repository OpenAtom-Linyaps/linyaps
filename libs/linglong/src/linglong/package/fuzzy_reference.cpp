...
utils::error::Result<FuzzyReference> FuzzyReference::parse(const QString &raw) noexcept
{
    ...
    if (not(matches.isValid() and matches.hasMatch())) {
        return LINGLONG_ERR("regexp mismatched.",static_cast<int>(utils::error::ErrorCode::Unknown));
    }
    ...
    if ((!versionStr.isEmpty()) && versionStr != "unknown") {
        ...
        if (!tmpVersion) {
            return LINGLONG_ERR(tmpVersion.error().message(),static_cast<int>(utils::error::ErrorCode::Unknown));
        }
    ...
    if ((!architectureStr.isEmpty()) && architectureStr != "unknown") {
        ...
        if (!tmpArchitecture) {
            return LINGLONG_ERR(tmpArchitecture.error().message(),static_cast<int>(utils::error::ErrorCode::Unknown));
        }
...
try {
    ...
    return LINGLONG_ERR(e.what(),static_cast<int>(utils::error::ErrorCode::Unknown));
}
