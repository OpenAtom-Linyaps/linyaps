  ...
  const api::types::v1::PackageManager1ExportParameters &params) noexcept
{
    ...
    if (!params.iconPath.empty()) {
        if (auto ret = packager.setIcon(QFileInfo{ QString::fromStdString(params.iconPath) });
            !ret) {
            taskContext.updateState(linglong::api::types::v1::State::Failed,
    ...
    if (layer->info()->kind != "app") {
        ...
        if (auto ret = packager.pack(exportPath.filename().c_str(), !params.full); !ret) {
            taskContext.updateState(linglong::api::types::v1::State::Failed,
    ...
    if (!params.full && layer->info()->runtime) {
        auto ret = packager.loadBlackList();
        if (!ret) {
            taskContext.updateState(linglong::api::types::v1::State::Failed,
                                    "load black list failed: " + ret.error().message());
            return;
        }
        ...
        if (!ret) {
            taskContext.updateState(linglong::api::types::v1::State::Failed,
                                    "load needed files failed: " + ret.error().message());
            return;
        }
    }
    ...
    if (!params.loader.empty()) {
        packager.setLoader(QString::fromStdString(params.loader));
    }
