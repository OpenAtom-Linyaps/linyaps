...
{
    LINGLONG_TRACE(
      fmt::format("exec cmd: {} args: {}", m_command, common::strings::join(args, ' ')));
    ...
    if (pipe(stdoutPipe) == -1) {
        return LINGLONG_ERR(fmt::format("pipe error: {}", strerror(errno)));
    }
    ...
    pid_t pid = fork();
    if (pid == -1) {
        return LINGLONG_ERR(fmt::format("fork error: {}", strerror(errno)));
    }
    ...
    if (!setNonBlock(stdoutPipe[0]) || !setNonBlock(stdinPipe[1])) {
        return LINGLONG_ERR(fmt::format("set non block error: {}", strerror(errno)));
    }

    int epfd = epoll_create1(O_CLOEXEC);
    if (epfd == -1) {
        return LINGLONG_ERR(fmt::format("epoll_create error: {}", strerror(errno)));
    }
    ...
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, stdoutPipe[0], &ev) == -1) {
        return LINGLONG_ERR(fmt::format("epoll_ctl stdout error: {}", strerror(errno)));
    }
    activeFds++;
    ...
    if (!m_stdinContent.empty()) {
        ...
        if (epoll_ctl(epfd, EPOLL_CTL_ADD, stdinPipe[1], &ev) == -1) {
            return LINGLONG_ERR(fmt::format("epoll_ctl stdin error: {}", strerror(errno)));
        }
        activeFds++;
    } else {
    ...
    while (activeFds > 0) {
        ...
        if (nfds == -1) {
            if (errno == EINTR)
                continue;
            return LINGLONG_ERR(fmt::format("epoll_wait error: {}", strerror(errno)));
        }
    ...
    while (waitpid(pid, &status, 0) == -1) {
        if (errno == EINTR)
            continue;
        return LINGLONG_ERR(fmt::format("waitpid error: {}", strerror(errno)));
    }
