...
utils::error::Result<void> InstallHookManager::executePostInstallHooks(
  const std::string &path, const std::string &appSignPath) noexcept
{
    ...
    if (-1 == setenv("LINGLONG_APP_SIGN_PATH", appSignPath.c_str(), 1)) {
        qWarning() << "failed to set LINGLONG_APP_SIGN_PATH" << errno;
    }

    for (const auto &command : postInstallCommands) {
        auto ret = std::system(command.c_str());
        if (!WIFEXITED(ret)) {
            qWarning() << "command did not terminate normally.";
            continue;
        }
    }
    ...
    if (::getenv("LINGLONG_APP_SIGN_PATH") != nullptr) {
        if (-1 == ::unsetenv("LINGLONG_APP_SIGN_PATH")) {
            qWarning() << "failed to unset LINGLONG_APP_SIGN_PATH" << errno;
        };
    }
