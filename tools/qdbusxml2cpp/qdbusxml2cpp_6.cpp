...
private:
    ...
    public:
        ...
        void warning(const QDBusIntrospection::SourceLocation &location,
                     const char *msg,
                     ...) override;
        void error(const QDBusIntrospection::SourceLocation &location,
                   const char *msg,
                   ...) override;
    ...
    private:
        ...
        void report(const QDBusIntrospection::SourceLocation &location,
                    const char *msg,
                    va_list ap,
                    const char *severity);
    };
    ...
    void writeSignalArgList(QTextStream &ts,
                            const QStringList &argNames,
                            const QDBusIntrospection::Annotations &annotations,
                                                ...
                                                const char *severity)
{
    fprintf(stderr,
            ...
            (long long int)location.columnNumber + 1,
            severity);
...
{
    const QLatin1StringView candidates[] = { "h"_L1, "cpp"_L1, "cc"_L1 };
...
static QString moc(const QString &name)
{
    ...
    if (fileNames.size() == 1) {
        ...
        if (isSupportedSuffix(fi.suffix())) {
            // Generates a file that contains the header and the implementation: include
            // "filename.moc"
...
void QDBusXmlToCpp::writeArgList(QTextStream &ts,
                                 const QStringList &argNames,
                                 const QDBusIntrospection::Annotations &annotations,
                                 const QDBusIntrospection::Arguments &inputArgs,
                                 const QDBusIntrospection::Arguments &outputArgs)
...
void QDBusXmlToCpp::writeSignalArgList(QTextStream &ts,
                                       const QStringList &argNames,
                                       const QDBusIntrospection::Annotations &annotations,
                                       const QDBusIntrospection::Arguments &outputArgs)
{
                               ...
                               const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces) {
        ...
        for (const QDBusIntrospection::Method &method : interface->methods) {
            bool isDeprecated =
              method.annotations.value("org.freedesktop.DBus.Deprecated"_L1).value == "true"_L1;
    ...
    if (!skipNamespaces) {
        ...
        do {
            ...
            // add this class:
            if (!name.isEmpty()) {
                hs << QString(current.size() * 2, u' ') << "using " << name
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces) {
        ...
        hs << "/*\n"
           ...
           << "\n"
              " */\n";
        cs << "/*\n"
              " * Implementation of adaptor class "
           << className
           << "\n"
              " */\n\n";
        ...
        for (const QDBusIntrospection::Property &property : interface->properties) {
            ...
            if (property.access != QDBusIntrospection::Property::Write) {
                ...
                cs << type << " " << className << "::" << getter
                   ...
                   << "\n"
                      "    return qvariant_cast< "
                   << type << " >(parent()->property(\"" << property.name
                   << "\"));\n"
                      "}\n\n";
            }
            ...
            if (property.access != QDBusIntrospection::Property::Read) {
                ...
                cs << "void " << className << "::" << setter << "(" << constRefType
                   << "value)\n"
                      "{\n"
        ...
        for (const QDBusIntrospection::Method &method : interface->methods) {
            ...
            if (isNoReply) {
                ...
                returnType = qtTypeName(method.outputArgs.first().location,
                                        method.outputArgs.first().type,
                                        method.annotations,
                                        0,
                                        "Out");
            ...
            cs << ")\n"
                  "{\n"
                  "    // handle method call "
               << interface->name << "." << methodName(method) << "\n";
            ...
            if (usingInvokeMethod) {
                ...
                if (!method.outputArgs.isEmpty())
                    cs << ", Q_RETURN_ARG("
                       << qtTypeName(method.outputArgs.at(0).location,
                ...
                for (qsizetype i = 0; i < method.inputArgs.size(); ++i)
                    cs << ", Q_ARG("
                       << qtTypeName(method.inputArgs.at(i).location,
                                     method.inputArgs.at(i).type,
                                     method.annotations,
                                     i,
                                     "In")
                       << ", " << argNames.at(i) << ")";
...
int QDBusXmlToCpp::run(const QCoreApplication &app)
{
    ...
    parser.addPositionalArgument(u"interfaces"_s,
                                 u"List of interfaces to use."_s,
                                 u"[interfaces ...]"_s);

    QCommandLineOption adapterCodeOption(QStringList{ u"a"_s, u"adaptor"_s },
                                         u"Write the adaptor code to <filename>"_s,
                                         u"filename"_s);
    parser.addOption(adapterCodeOption);

    QCommandLineOption classNameOption(
      QStringList{ u"c"_s, u"classname"_s },
    ...
    QCommandLineOption addIncludeOption(QStringList{ u"i"_s, u"include"_s },
                                        u"Add #include \"filename\" to the output"_s,
                                        u"filename"_s);
    ...
    QCommandLineOption adapterParentOption(
      u"l"_s,
      u"When generating an adaptor, use <classname> as the parent class"_s,
      u"classname"_s);
    parser.addOption(adapterParentOption);

    QCommandLineOption mocIncludeOption(
      QStringList{ u"m"_s, u"moc"_s },
    ...
    QCommandLineOption proxyCodeOption(QStringList{ u"p"_s, u"proxy"_s },
                                       u"Write the proxy code to <filename>"_s,
                                       u"filename"_s);
    parser.addOption(proxyCodeOption);

    QCommandLineOption verboseOption(QStringList{ u"V"_s, u"verbose"_s }, u"Be verbose."_s);
